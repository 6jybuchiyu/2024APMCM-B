...
if __name__ == '__main__':
    ...
    labels = minibatch_kmeans.fit_predict(flood_probabilities)
    ...
    plt.xlabel('采样点')
    plt.ylabel('洪水概率')
    plt.title('洪水概率因素的Mini-Batch K-means聚类散点图')
    plt.legend()
    plt.show()

    # 将聚类类别添加到原始数据文件的最后一列并保存
    train.to_csv('./processed_train_data_minibatch_kmeans.csv', index=False)
